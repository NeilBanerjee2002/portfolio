export default function Navbar({ t, setLang }) {
    return (
        <nav className='sticky top-0 z-50 backdrop-blur-xl bg-white/5 border-b border-white/10 px-8 py-4 flex items-center justify-between'>
            ...
            <div className='flex gap-2 text-xl'><button onClick={() => setLang("de")} className='hover:scale-110'>🇩🇪</button><button onClick={() => setLang("en")} className='hover:scale-110'>🇬🇧</button></div>
        </nav>
    )
}